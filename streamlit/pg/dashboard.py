...
if "league" in df.columns:
    ...
    if "gamelength" in lck_teams.columns:
        ...
        if "year" in lck_teams.columns and "teamname" in lck_teams.columns:
            ...
            ax.barh(
                yearly_data["teamname"],
                yearly_data["gamelength_minutes"],
                color="skyblue",
            )
            ax.set_title(
                f"LCK 팀별 평균 Game Length (Year: {selected_year})", fontsize=16
            )
    ...
    if "goldspent" in lck_teams.columns:
        ...
        if "year" in lck_teams.columns and "teamname" in lck_teams.columns:
            ...
            selected_year = st.selectbox(
                "연도를 선택하세요:", available_years, key="lck_year_selectbox"
            )
            ...
            ax.set_title(
                f"LCK 팀별 Gold Spent 평균 (Year: {selected_year})", fontsize=16
            )
...
if "league" in df.columns:
    ...
    if {"kills", "deaths", "assists"}.issubset(lck_teams.columns):
        ...
        if "year" in lck_teams.columns and "teamname" in lck_teams.columns:
            ...
            # KDA 계산
            lck_grouped["KDA"] = (lck_grouped["kills"] + lck_grouped["assists"]) / (
                lck_grouped["deaths"] + 1
            )

            # 사용 가능한 연도 선택
            available_years = sorted(lck_grouped["year"].unique())
            selected_year = st.selectbox(
                "연도를 선택하세요:", available_years, key="year_selectbox_kda"
            )
...
if "league" in df.columns:
    ...
    if "golddiffat15" in lck_teams.columns:
        ...
        if "year" in lck_teams.columns and "teamname" in lck_teams.columns:
            ...
            available_years = sorted(lck_grouped["year"].unique())
            selected_year = st.selectbox(
                "연도를 선택하세요:", available_years, key="year_selectbox_gold"
            )
            ...
            # 시각화
            st.subheader(
                f"LCK 팀별 Gold Difference at 15 Minutes 평균 (Year: {selected_year})"
            )
            fig, ax = plt.subplots(figsize=(10, 8))
            ax.barh(
                yearly_data["teamname"], yearly_data["golddiffat15"], color="skyblue"
            )
            ax.set_title(
                f"LCK 팀별 Gold Difference at 15 Minutes 평균 (Year: {selected_year})",
                fontsize=16,
...
selected_year = st.selectbox(
    "연도를 선택하세요:", available_years, key="year_selectbox_dragon"
)
...
if {"teamname", "result", "firstdragon", "dragons"}.issubset(lck_year.columns):
    ...
    # 시각화
    st.subheader(
        f"{selected_year}년 LCK 팀별 First Dragon 및 Dragons 평균 (승리 횟수 순)"
    )
...
available_years = sorted(df["year"].unique())
selected_year = st.selectbox(
    "연도를 선택하세요:", available_years, key="year_selectbox_gold_diff"
)
...
if {"teamname", "result", "golddiffat10", "golddiffat15"}.issubset(lck_year.columns):
    ...
    # 팀별 골드 차이 평균 계산
    gold_diff_stats = lck_year.groupby("teamname")[
    ...
    # 시각화
    st.subheader(
        f"{selected_year}년 LCK 팀별 Gold Difference (10분 및 15분) 평균 (승리 횟수 순)"
    )
    fig, ax = plt.subplots(figsize=(12, 6))
    ...
    st.error(
        "데이터에 'teamname', 'result', 'golddiffat10', 'golddiffat15' 열이 없습니다."
    )
...
available_years = sorted(df["year"].unique())
selected_year = st.selectbox(
    "연도를 선택하세요:", available_years, key="year_selectbox_xp_diff"
)
...
if {"teamname", "result", "xpdiffat10", "xpdiffat15"}.issubset(lck_year.columns):
    ...
    # 시각화
    st.subheader(
        f"{selected_year}년 LCK 팀별 XP Difference (10분 및 15분) 평균 (승리 횟수 순)"
    )
    fig, ax = plt.subplots(figsize=(12, 6))
...
# LCK 팀별 DPM 평균
selected_year = st.selectbox(
    "연도를 선택하세요:", available_years, key="year_selectbox_dpm"
)
...
available_years = sorted(df["year"].unique())
selected_year = st.selectbox(
    "연도를 선택하세요:", available_years, key="year_selectbox_vspm"
)
...
if "league" in df.columns and "split" in df.columns:
    ...
    gamelength_by_split = (
        lck_data.groupby(["year", "split"])["gamelength_minutes"].mean().unstack()
    )
